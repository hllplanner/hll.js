...
class BaseManager {
   ...
   * @param {boolean} [conditions.integer=false] - Parameter must be an integer.
   * @param {boolean} [conditions.boolean=false] - Parameter must be a boolean.
   */
  _validateParameter(value, name, conditions = {
    defined: true, nonEmptyString: true, positiveInteger: false, integer: false, boolean: false
  }) {
    ...
    if (conditions.integer && !Number.isInteger(value)) {
      throw new Error(`Validation Error: '${name}' must be an integer.`);
    }
